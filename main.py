...
class DIPStudioUltra(ctk.CTk):
    def __init__(self):
        ...
        self.blend_img2 = None # Second image storage
    ...
    def _setup_ui(self):
        ...
        self.viewport.pack(expand=True, fill="both", pady=(0, 100))

        # 4. HUD OVERLAY (Color fixed to 6-digits)
        self.hud = ctk.CTkFrame(self.workspace, height=80, fg_color="#0D0D0D", corner_radius=15, border_width=1, border_color=self.clr_border)
        self.hud.place(relx=0.5, rely=0.95, anchor="s", relwidth=0.8)
        
        self.fig, self.ax = plt.subplots(figsize=(4, 1.0))
        self.fig.patch.set_facecolor("none")
        self.canvas_hist = FigureCanvasTkAgg(self.fig, master=self.hud)
        self.canvas_hist.get_tk_widget().pack(side="left", padx=20, pady=(5, 20))
    ...
    def create_action_btn(self, parent, text, cmd, is_warn=False):
        ...
        # FIX: Removed 'padx' from constructor
    ...
    def update_tool_ui(self):
        ...
        if self.current_mode == "ADJUST":
            ...
            # Distinct lambdas to prevent the "All Buttons Extract Red" bug
            ...
            # Fix: Added missing Import Second Pic button
            self.create_action_btn(self.scroll_area, "Load Second Image", self.load_second_buffer)
    ...
    def show_image(self, img, is_preview=False):
        ...
        if render_img.ndim == 3:
            for i, col in enumerate(("#FF0000", "#00FF1A", "#000DFF")):
                h = cv2.calcHist([render_img], [i], None, [256], [0, 256])
                self.ax.plot(h.flatten(), color=col, lw=0.6, alpha=0.9)
            rgb = cv2.cvtColor(render_img, cv2.COLOR_BGR2RGB)
        else:
            h = cv2.calcHist([render_img], [0], None, [256], [0, 256])
            self.ax.plot(h.flatten(), color=self.clr_accent, lw=0.6, alpha=0.4)
